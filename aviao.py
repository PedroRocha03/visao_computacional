...
def processar_imagem(nome_arquivo):
    ...
    kernel_edges = np.ones((5, 5), np.uint8)
    edges = cv2.dilate(edges, kernel_edges, iterations=1)

    contours, _ = cv2.findContours(image=edges.copy(), mode=cv2.RETR_EXTERNAL, method=cv2.CHAIN_APPROX_SIMPLE)
    contours = sorted(contours, key=cv2.contourArea, reverse=True)[:6]

    img_contours = img.copy()
    cv2.drawContours(img_contours, contours, contourIdx=-1, color=(255, 0, 0), thickness=3)
